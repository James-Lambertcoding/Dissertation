...
vector_fun <- function(start_point, size){
  ...
  test_points[6,3] <- test_points[6,3] - size
...
test_df <- vector_fun(start_point = start_test,size = 0.8)
...
## create a step 27 check
check_27_step <- 0
check_no_change <- 0

mega_function <- function(steps,start_loc,the_big_list, temp_sectors,function_parameters){
  ...
  for(q in 1:length(temp_sectors)){
    ...
    for(k in 1:steps[1]){
      ...
      temp_data_df[k,7] <- temp_step_size[k,2]
      
      
      
      if(k>1){
        
        temp_data_df[k,6] <- (temp_data_df[k-1,4]-temp_data_df[k,4])/temp_data_df[k-1,4]
        
      }
      
      temp_start <- temp_results[1:3]
      
      ## check to see if repeating steps
      if(temp_results[[1,5]]==27){
        
        check_27_step <- check_27_step +1
      }
      ## check to see if change is small
      if(abs(temp_data_df[k,6])<function_parameters[5]){
        
        check_no_change <- check_no_change+1
        
      }
      
      
       if(check_27_step > function_parameters[1]){
        
        temp_step_size[,2] <- temp_step_size[,2]/function_parameters[3]
        
        check_27_step <- 0
       } 
       
         if(check_no_change>function_parameters[2]){
           
           temp_step_size[,2] <- temp_step_size[,2]*function_parameters[4]
           check_no_change <- 0
         }
...
## remove 40 sector
sectors_need_2 <- sectors_need[-length(sectors_need)]
secotr_need_small <- sectors_need[3]

## function_parameters
## 1: check_27_step time
## 2: check_no_change time
## 3: check_27 decrease
## 4: check_no increase
## 5: check_no size
fun_para_mega <- c(2,8,5,4,0.02)

## 4.2 Run ---------------
test_2 <- mega_function(steps =search_steps_no, 
                            start_loc = start_loc,
                            the_big_list =big_list,
                            temp_sectors = secotr_need_small,
                            function_parameters = fun_para_mega
)
