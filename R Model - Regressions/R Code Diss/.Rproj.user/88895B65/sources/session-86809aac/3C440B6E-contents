...
for(i in 1:length(good_types)){
  ...
  scatter_plots_log[[i]] <- ggplot(split_list[[i]], aes(x=Income.Amount, y = Spend))+
    geom_point(color = "darkblue") +
    theme_economist_white() + 
    scale_colour_economist()+
    xlab("Total Monthly Spend (2008 Prices)") +
    ylab("Monthly Spend on Good (2008 Prices)")+
...
for(i in 1:length(good_types)){
  ...
  scatter_plots_line[[i]] <- ggplot(split_list[[i]], aes(x=Income.Amount, y = Spend))+
    geom_point(color = "darkblue") +
    theme_economist_white() + 
    scale_colour_economist()+
    xlab("Total Monthly Spend (2008 Prices)") +
...
for(i in 1:length(good_types)){
  ...
  scatter_plots_price[[i]] <- ggplot(split_list[[i]], aes(x=Price, y = Spend, color = Income.Desc))+
    geom_point()+
   # geom_point(color = "darkblue") +
    theme_economist_white() + 
    #scale_colour_economist()+
...
for(i in 1:length(good_types)){
  ...
  scatter_plots_income[[i]] <- ggplot(split_list[[i]], aes(x=Income.Amount, y = Spend))+
    geom_point(color = "darkblue") +
    theme_economist_white() + 
    scale_colour_economist()+
    xlab("Total Monthly Spend (2008 Prices)") +
...
scatter_plots_income[[2]]

## plots 
scatter_plots_log[[10]]
...
coef_lasso <- data.frame(matrix(ncol = length(good_types), nrow = 14))
coef_ridge <- data.frame(matrix(ncol = length(good_types), nrow = 14))
...
for(i in 1:length(good_types)){
  ...
  coef_lasso[,i] <- coef(best_model_lasso[[i]])[1:14]
  coef_ridge[,i] <- coef(best_model_ridge[[i]])[1:14]
...
## Coefs
## ML
write.csv(x = coef_lasso,file = "Output_2/coef_LASSO.csv")
write.csv(x = coef_ridge,file = "Output_2/coef_Ridge.csv")
## Linear
...
for(i in 1:length(good_types)){
  ...
  # ggplot2::ggsave(filename = paste0("Output_2/Plots Linear Income/plot_lin_",i,".png"),plot = scatter_plots_line[[i]])
  # ggplot2::ggsave(filename = paste0("Output_2/Plots Log Income/plot_lin_",i,".png"),plot = scatter_plots_log[[i]])
  # ggplot2::ggsave(filename = paste0("Output_2/Plots Price/plot_lin_",i,".png"),plot = scatter_plots_price[[i]])
  ggplot2::ggsave(filename = paste0("Output_2/Plots No Regression Lines/plot_income_",i,".png"),plot = scatter_plots_income[[i]])
}
